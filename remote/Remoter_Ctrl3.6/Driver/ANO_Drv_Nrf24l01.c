...
uint8_t ANO_NRF_Read_Reg(uint8_t reg)
{
	...
	reg_val = ANO_SPI_RW(0xff);	  /* 读取该寄存器返回数据 */
	ANO_SPI_CSN_H();					  /* 禁止该器件 */
   return 	reg_val;
}
...
u8 ANO_NRF_TxPacket(uint8_t * tx_buf, uint8_t len)
{	
	...
	ANO_SPI_CE_L();
  	ANO_NRF_Write_Buf(WR_TX_PLOAD,tx_buf,TX_PLOAD_WIDTH);//Ð´Êý¾Ýµ½TX BUF  32¸ö×Ö½Ú
 	ANO_SPI_CE_H();	   
//	while(GPIO_ReadInputDataBit(NRF_IRQ_GPIO, NRF_IRQ_Pin)!=0);//µÈ´ý·¢ËÍÍê³É
	while(NRF24L01_IRQ!=0);
	sta=ANO_NRF_Read_Reg(NRFRegSTATUS);  //¶ÁÈ¡×´Ì¬¼Ä´æÆ÷µÄÖµ	   
	ANO_NRF_Write_Reg(NRF_WRITE_REG+NRFRegSTATUS,sta); //Çå³ýTX_DS»òMAX_RTÖÐ¶Ï±êÖ¾
	if(sta&MAX_TX)//´ïµ½×î´óÖØ·¢´ÎÊý
	{
		ANO_NRF_Write_Reg(FLUSH_TX,0xff);//Çå³ýTX FIFO¼Ä´æÆ÷ 
		return MAX_TX; 
	}
	if(sta&TX_OK)//·¢ËÍÍê³É
	{
...
void NRF_Check_Ch(void)  //对频
{
	...
	//{
		...
		//else
		//{
			ANO_NRF_Write_Reg(NRF_WRITE_REG+RF_CH, 0); //设置频率信道
...
void ANO_NRF_Init(u8 model, u8 ch)
{
	...
	ANO_NRF_Write_Reg(NRF_WRITE_REG+RF_CH,0);														//设置RF通道为CHANAL
...
//	}
	...
	ANO_NRF_Write_Reg(NRF_WRITE_REG+CONFIG,0x0e);    //ÅäÖÃ»ù±¾¹¤×÷Ä£Ê½µÄ²ÎÊý;PWR_UP,EN_CRC,16BIT_CRC,½ÓÊÕÄ£Ê½,¿ªÆôËùÓÐÖÐ¶Ï
